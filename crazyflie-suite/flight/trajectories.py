"""
Provides functions that return trajectories in form of a list of setpoints.
Author: S. Pfeiffer, MAVLab
"""

import numpy as np


def takeoff(x, y, altitude, yaw):
    setpoints = [(x, y, altitude, yaw,1)]

    return setpoints


def landing(x, y, altitude, yaw):
    setpoints = [(x, y, altitude, yaw,1), (x, y, 0, yaw, 1)]

    return setpoints


def scan_area(x_bound, y_bound, spacing, altitude, yaw):
    x_min, x_max = x_bound
    y_min, y_max = y_bound

    setpoints = []
    x_range = x_max - x_min
    n_lines = int(np.floor(x_range / spacing) + 1)
    y_is_min = True

    for i in range(n_lines):
        x_line = x_min + i * spacing
        y = y_min if y_is_min else y_max
        setpoints.append((x_line, y, altitude, yaw))
        y_is_min = not (y_is_min)
        y = y_min if y_is_min else y_max
        setpoints.append((x_line, y, altitude, yaw))

    return setpoints


def xy_square(x0, y0, side_length, altitude, yaw):
    x_min = x0 - side_length / 2
    x_intermin = x0 - (side_length / 2) + (side_length / 8)
    x_intermax =  x0 + (side_length / 2) - (side_length / 8)
    x_max = x0 + side_length / 2
    y_min = y0 - side_length / 2
    y_max = y0 + side_length / 2

    setpoints = [
        (x_min, y_min, altitude, yaw,1),
        (x_min, y_max, altitude, yaw,0),

      #  (x_intermin, y_max, altitude, yaw,0),
      #  (x_intermax, y_max, altitude, yaw,1),

        (x_max, y_max, altitude, yaw,1),
        (x_max, y_min, altitude, yaw,0),

      #  (x_intermax, y_min, altitude, yaw,0),
      #  (x_intermin, y_min, altitude, yaw,1),

        (x_min, y_min, altitude, yaw,1),

        
    ]

    return setpoints

def tinyxy_square(x0, y0, side_length, altitude, yaw):
    x_min = x0 - side_length / 2
    x_intermin = x0 - (side_length / 2) + (side_length / 8)
    x_intermax =  x0 + (side_length / 2) - (side_length / 8)
    x_max = x0 + side_length / 2
    y_min = y0 - side_length / 2
    y_max = y0 + side_length / 2

    setpoints = [
        (x_min, y_min, altitude, yaw,0),
        (x_min, y_max, altitude, yaw,0),

      #  (x_intermin, y_max, altitude, yaw,0),
      #  (x_intermax, y_max, altitude, yaw,1),

        (x_max, y_max, altitude, yaw,0),
        (x_max, y_min, altitude, yaw,0),

      #  (x_intermax, y_min, altitude, yaw,0),
      #  (x_intermin, y_min, altitude, yaw,1),

        (x_min, y_min, altitude, yaw,0),

        
    ]

    return setpoints

def xy_square1(x0, y0, side_length, altitude, yaw):
    x_min = x0 - side_length / 2
    x_intermin = x0 - (side_length / 2) + (side_length / 8)
    x_intermax =  x0 + (side_length / 2) - (side_length / 8)
    x_max = x0 + side_length / 2
    y_min = y0 - side_length / 2
    y_max = y0 + side_length / 2

    setpoints = [
        (x_min, y_max, altitude, yaw,0),

       # (x_intermin, y_max, altitude, yaw,0),
       # (x_intermax, y_max, altitude, yaw,1),

        (x_max, y_max, altitude, yaw,1),
        (x_max, y_min, altitude, yaw,0),

      #  (x_intermax, y_min, altitude, yaw,0),
      #  (x_intermin, y_min, altitude, yaw,1),

        (x_min, y_min, altitude, yaw,1),
        (x_min, y_max, altitude, yaw,0),
    ]

    return setpoints


def xy_square2(x0, y0, side_length, altitude, yaw):
    x_min = x0 - side_length / 2
    x_intermin = x0 - (side_length / 2) + (side_length / 8)
    x_intermax =  x0 + (side_length / 2) - (side_length / 8)
    x_max = x0 + side_length / 2
    y_min = y0 - side_length / 2
    y_max = y0 + side_length / 2

    setpoints = [
        (x_max, y_max, altitude, yaw,1),
        (x_max, y_min, altitude, yaw,0),
        
       # (x_intermax, y_min, altitude, yaw,0),
       # (x_intermin, y_min, altitude, yaw,1),

        (x_min, y_min, altitude, yaw,1),
        (x_min, y_max, altitude, yaw,0),

        #(x_intermin, y_max, altitude, yaw,0),
        #(x_intermax, y_max, altitude, yaw,1),

        (x_max, y_max, altitude, yaw,1),
    ]

    return setpoints

def xy_square3(x0, y0, side_length, altitude, yaw):
    x_min = x0 - side_length / 2
    x_intermin = x0 - (side_length / 2) + (side_length / 8)
    x_intermax =  x0 + (side_length / 2) - (side_length / 8)
    x_max = x0 + side_length / 2
    y_min = y0 - side_length / 2
    y_intermin = y0 - (side_length / 2) + (side_length / 8)
    y_intermax = y0 + side_length / 2 - (side_length / 8)
    y_max = y0 + side_length / 2

    setpoints = [
        (x_max, y_min, altitude, yaw,0),

       # (x_intermax, y_min, altitude, yaw,0),
       # (x_intermin, y_min, altitude, yaw,1),

        (x_min, y_min, altitude, yaw,1),
        (x_min, y_max, altitude, yaw,0),

       # (x_intermin, y_max, altitude, yaw,0),
       # (x_intermax, y_max, altitude, yaw,1),

        (x_max, y_max, altitude, yaw,1),
        (x_max, y_min, altitude, yaw,0),
    ]

    return setpoints
def xy_square_fw(x0, y0, side_length, altitude, yaw0):
    x_min = x0 - side_length / 2
    x_max = x0 + side_length / 2
    y_min = y0 - side_length / 2
    y_max = y0 + side_length / 2

    setpoints = [
        (x_min, y_min, altitude, yaw0),
        (x_min, y_min, altitude, yaw0 + 90),
        (x_min, y_max, altitude, yaw0 + 90),
        (x_min, y_max, altitude, yaw0),
        (x_max, y_max, altitude, yaw0),
        (x_max, y_max, altitude, yaw0 - 90),
        (x_max, y_min, altitude, yaw0 - 90),
        (x_max, y_min, altitude, yaw0),
    ]

    return setpoints


def xy_hourglass(x0, y0, side_length, altitude, yaw):
    x_min = x0 - side_length / 2
    x_max = x0 + side_length / 2
    y_min = y0 - side_length / 2
    y_max = y0 + side_length / 2

    setpoints = [
        (x_max, y_max, altitude, yaw),
        (x_max, y_min, altitude, yaw),
        (x_min, y_max, altitude, yaw),
        (x_min, y_min, altitude, yaw),
    ]

    return setpoints


def xy_polygon(x0, y0, N, radius, altitude, yaw):
    setpoints = []
    for i in range(N):
        angle = 2 * np.pi * i / N
        x = radius * np.cos(angle)
        y = radius * np.sin(angle)
        setpoints.append((x0 + x, y0 + y, altitude, yaw))
    setpoints.append(setpoints[0])

    return setpoints

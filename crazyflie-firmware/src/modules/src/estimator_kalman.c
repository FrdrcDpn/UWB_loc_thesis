...
void estimatorKalmanGetEstimatedK(point_t* kval) {
  kval->x = 10*coreData.P[KC_STATE_X][KC_STATE_X];//swarmPx;
  kval->y = 10*coreData.P[KC_STATE_Y][KC_STATE_Y];//swarmPy;
  kval->z = 10*coreData.P[KC_STATE_X][KC_STATE_Y];//swarmPxy;
}

void estimatorKalmanGetEstimatedRot(float * rotationMatrix) {
  memcpy(rotationMatrix, coreData.R, 9*sizeof(float));
}
void estimatorKalmanGetSwarmInfo(float* vx, float* vy, float* vz, float* kx, float* ky, float* kz, float* gyroZ, float* height) {
  *vx = coreData.S[KC_STATE_X];//swarmVx;
  *vy = coreData.S[KC_STATE_Y];//swarmVy;
  *vz = coreData.S[KC_STATE_Z];//swarmVz;
  *kx = 10*coreData.P[KC_STATE_X][KC_STATE_X];//swarmPx;
  *ky = 10*coreData.P[KC_STATE_Y][KC_STATE_Y];//swarmPy;
  *kz = 10*coreData.P[KC_STATE_X][KC_STATE_Y];//swarmPxy;
  *gyroZ = swarmGz;
  *height = swarmh;
}

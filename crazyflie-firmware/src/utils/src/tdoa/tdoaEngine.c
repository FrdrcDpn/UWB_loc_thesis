...
static lpstdoaoptions tdoaOptions = {
.userRequestedtdoamode = tdoa_enable,
...
void tdoaEngineProcessPacket(tdoaEngineState_t* engineState, tdoaAnchorContext_t* anchorCtx, const int64_t txAn_in_cl_An, const int64_t rxAn_by_T_in_cl_T) {
  ...
  if (timeIsGood) {
    ...
    if (findSuitableAnchor(engineState, &otherAnchorCtx, anchorCtx)) {
      ...
      if (tdoaOptions.currenttdoamode != tdoaOptions.userRequestedtdoamode) {
        tdoaOptions.currenttdoamode = tdoaOptions.userRequestedtdoamode;
        DEBUG_PRINT("Enabled/disabled tdoa:  %d\n", tdoaOptions.currenttdoamode);
      } 
      // our vanilla ranging scheme
         if(tdoaOptions.currenttdoamode == tdoa_enable){
...
LOG_GROUP_START(tdoa_mode)
LOG_ADD_CORE(LOG_UINT8, tdoa_modus,&tdoaOptions.currenttdoamode)
LOG_GROUP_STOP(tdoa_mode)

PARAM_GROUP_START(tdoa_mode)
PARAM_ADD_CORE(PARAM_UINT8, tdoa_modus, &tdoaOptions.userRequestedtdoamode)
PARAM_GROUP_STOP(tdoa_mode)
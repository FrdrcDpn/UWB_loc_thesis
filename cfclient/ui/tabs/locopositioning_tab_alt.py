...
__all__ = ['LocoPositioningTab_alt']

logger = logging.getLogger(__name__)

locopositioning_tab_class = uic.loadUiType(
    cfclient.module_path + "/ui/tabs/locopositioning_tab_alt.ui")[0]
...
class AnchorStateMachine:
    ...
    def _get_mem(self, mem_sub):
        mem = mem_sub.get_mems(MemoryElement.TYPE_LOCO2_ALT)
        if len(mem) > 0:
            return mem[0]
        return None
...
class LocoPositioningTab_alt(Tab, locopositioning_tab_class):
    ...
    PARAM_MDOE_GR = 'loco_alt'
    PARAM_MODE_NM = 'mode_alt'
    ...
    def __init__(self, tabWidget, helper, *args):
        super(LocoPositioningTab_alt, self).__init__(*args)
        self.setupUi(self)

        self.tabName = "Loco Positioning alt"
        self.menuName = "Loco Positioning Tab alt"
